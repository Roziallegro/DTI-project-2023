...
int NUMSTRIPS = 3; // total there's 3 strips
...
void light_0(){
  for (int p=0; p<NUMPIXELS; p++){
    strips[0].setPixelColor(p, strips[0].Color(255,219,165)); // colour: sunbean
  }
  strips[0].show();
...
void light_2(){
  for (int p=0; p<NUMPIXELS; p++){
    strips[2].setPixelColor(p, strips[2].Color(220,20,60)); // colour: crimson
  }
  strips[2].show();
...
void light_run(){
  for (int k=0; k<NUMSTRIPS; k++){
    for (int p=0; p<NUMPIXELS-1; p++){
      strips[k].setPixelColor(p, strips[k].Color(220,20,60)); // colour: crimson
      strips[k].show();
      delay(100);
      strips[k].setPixelColor(p+1, strips[k].Color(220,20,60));
      strips[k].show();
      delay(100);
