...
int NUMSTRIPS = 4; // total there's 2 strips
...
void light_0(){
  for (int p=0; p<NUMPIXELS; p++){
    strips[0].setPixelColor(p, strips[0].Color(218,165,32)); // colour: goldenrod
  }
  strips[0].show();
...
void light_1(){
  for (int p=0; p<NUMPIXELS; p++){
    strips[1].setPixelColor(p, strips[1].Color(225,20,147)); // colour: deeppink 
  }
  strips[1].show();
...
void light_2(){
  for (int p=0; p<NUMPIXELS; p++){
    strips[2].setPixelColor(p, strips[2].Color(210,105,30)); // colour: chocolate 
  }
  strips[2].show();
...
void light_3(){
  for (int p=0; p<NUMPIXELS; p++){
    strips[3].setPixelColor(p, strips[3].Color(255,243,225)); // colour: almond
  }
  strips[3].show();
  Serial.println("light 3 lit");
}
...
void light_run(){
  for (int k=0; k<NUMSTRIPS; k++){
    for (int p=0; p<NUMPIXELS-1; p++){
      strips[k].setPixelColor(p, strips[k].Color(210,105,30)); // colour: chocolate
      strips[k].show();
      delay(100);
      strips[k].setPixelColor(p+1, strips[k].Color(210,105,30));
      strips[k].show();
      delay(100);
